// --- src/game.js (Lógica Principal) ---
        ...
        class HUDProjectile {
            constructor(scene, startPosition, type) {
                ...
                if (type === 'health') {
                    ...
                    this.element.style.boxShadow = '0 0 10px #00aaff';
                } else {
                    // Soul/Spectral
                    this.element.style.backgroundColor = '#ff00ff'; // Purple/Magenta
                    this.element.style.boxShadow = '0 0 10px #ff00ff';
                    // If available, use an image:
                    // this.element.style.backgroundImage = `url('${assetUrls.soulFragment}')`;
                }

                document.body.appendChild(this.element);

                // Calculate screen position from 3D start position
                const screenPos = this.getScreenPosition(startPosition);
                this.currentX = screenPos.x;
                this.currentY = screenPos.y;

                this.element.style.left = `${this.currentX}px`;
                this.element.style.top = `${this.currentY}px`;

                // Target Element
                let targetSelector = '#energy-bar';
                if (type === 'power') targetSelector = '#power-bar';
                if (type === 'soul') targetSelector = '#spectral-bar'; // New spectral bar ID
                ...
                this.speed = 0.05; // Initial interpolation factor
                this.progress = 0;
            ...
            update(deltaTime) {
                ...
                this.progress += deltaTime * 2.0; // Speed multiplier

                // Ease In-Out
                const t = Math.min(this.progress, 1.0);
                const ease = t * t * (3 - 2 * t);

                const x = this.currentX + (this.targetX - this.currentX) * ease;
                const y = this.currentY + (this.targetY - this.currentY) * ease;
            ...
            applyEffect() {
                ...
                if (this.type === 'soul') {
                    // Restore spectral/soul meter logic here if exists, or just counter
                    // Assuming spectral bar logic exists or needs to be added
                    // player.restoreSpectral(5);
                }
            }
        ...
        function spawnLoot(scene, x, y, z) {
            ...
            // Regular Drops
            // 60% Soul (Purple/Common)
            // 20% Health (Green/Rare)
            // 20% Power (Blue/Rare)

            let type = 'soul';
            if (rand < 0.25) { // 20% + 5% covered above, simplified: range 0.05 to 0.25 is 20% range
                type = 'health';
            } else if (rand < 0.45) { // Another 20%
                type = 'power';
            } else {
                type = 'soul';
            }
